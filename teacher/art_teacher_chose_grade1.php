...
<?php
	$sql = "SELECT * FROM ".$ART_TABLE."instrument_student_select 
			...
			WHERE teacher = '".$teacher_id."' AND finally = '".$instrument."' AND ".$ART_TABLE."instrument_student_select.year = '".$year."' AND ".$TABLE."major.id= '".$major."' ";
	//echo $sql;
...
if(($third <= 0  && $volunteer == 3)  ||  $pass )  //第三志愿选择完后或者时间已经过了
{
	...
	$sql = "SELECT  ".$TABLE."student_sheet.name AS student_name , ".$TABLE."student_sheet.class AS student_class ,mobilephone , short_number ,student_number 
		...
		LEFT JOIN ".$TABLE."major ON ".$TABLE."student_sheet.profession = ".$TABLE."major.name 
		WHERE ".$ART_TABLE."instrument_student_select.year = '".$year."' AND teacher = '".$teacher_id."' AND bysj_major.id ='".$major."'";
		//echo $sql;
	$query =mysql_query($sql);
	if(mysql_num_rows($query))
	{
...
$(document).ready(function(){
	$(".delete").click( function () { 
		var id = $(this).attr("id")
			$.post("./ajax/delete_volunteer.php", { number: id} ,function(data){
   				//alert("Data Loaded: " + data);
   				window.location.href="./art_teacher_chose_grade1.php?select_year=<?php echo $year;?>&major_id=<?php echo $instrument;?>&class=<?php echo $major; ?>";
   				});
		});
});
