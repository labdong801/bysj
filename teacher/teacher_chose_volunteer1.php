...
if($_SESSION['com_id']) //检查是否登录
 {
	...
	{ 
		...
		else if($volunteer == 2)
		{
			...
			//该乐器所有教师一共可以带多少个学生\
//			$volunteer_sum = 0;
//			$sql= "SELECT * FROM  `".$ART_TABLE."teacher_student` WHERE major_id = '".$instrument."' AND year = '".$year."' AND  class ='".$major."' ";
//			$query = mysql_query($sql);
//			if(mysql_num_rows($query))
//			{
//				while($row = mysql_fetch_array($query))
//					$volunteer_sum+=$row['value'];
//			}
//			echo $volunteer_sum;
		}
		else
...
//			mysql_query($sql);
	 ...
	 $sql = "SELECT ".$TABLE."student_sheet.name , ".$ART_TABLE."instrument_student_select.student_number, ".$TABLE."student_sheet.class, ".$ART_TABLE."instrument_student_select.lock,".$ART_TABLE."instrument_student_select.first,".$ART_TABLE."instrument_student_select.second,".$ART_TABLE."instrument_student_select.third  FROM  `".$ART_TABLE."instrument_student_select` 
	 	...
	 	while($row = mysql_fetch_array($query))
	 	{
	 		 if($volunteer == "second")
	 		 {
	 		 	...
	 		 	if(mysql_num_rows($query_check))
	 		 		while($row_check = mysql_fetch_array($query_check))
	 		 			$sum += $row_check['value'];
	 		 	//echo $sum;
	 		 	$sql_check = "SELECT * FROM  `".$ART_TABLE."instrument_student_select` 
	 		 			LEFT JOIN ".$TABLE."student_sheet ON ".$ART_TABLE."instrument_student_select.student_number = ".$TABLE."student_sheet.number  
	 					LEFT JOIN ".$TABLE."major ON ".$TABLE."student_sheet.profession = ".$TABLE."major.name
	 		 			WHERE finally = '".$row['first']."'  AND ".$ART_TABLE."instrument_student_select.year = '".$year."' AND  ".$TABLE."major.id='".$major."' ";
	 		 	$query_check = mysql_query($sql_check);
	 		 ...
	 		 if($volunteer == "third")
	 		 {
	 		 	...
	 		 	$sql_check = "SELECT * FROM  `".$ART_TABLE."instrument_student_select` 
	 		 			LEFT JOIN ".$TABLE."student_sheet ON ".$ART_TABLE."instrument_student_select.student_number = ".$TABLE."student_sheet.number  
	 					LEFT JOIN ".$TABLE."major ON ".$TABLE."student_sheet.profession = ".$TABLE."major.name
	 		 			WHERE finally = '".$row['second']."'  AND ".$ART_TABLE."instrument_student_select.year = '".$year."' AND  ".$TABLE."major.id='".$major."' ";
	 		 	$query_check = mysql_query($sql_check);
	 		 	$chose_sum = mysql_num_rows($query_check);
	 		 	//echo $chose_sum;
	 		 	//公式：上一志愿还剩多少个名额 = 上一志愿教师可以带的人数 - 上一志愿已选择的人数；
