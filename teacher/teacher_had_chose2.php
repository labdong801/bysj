...
if($_SESSION['com_id']) //检查是否登录
 {
	...
	{ 
	 ...
	 $sql = "SELECT ".$TABLE."student_sheet.name , ".$ART_TABLE."vocalmusic_student_select.student_number, ".$TABLE."student_sheet.class, ".$ART_TABLE."vocalmusic_student_select.".$instrument."_lock AS `lock`  FROM  `".$ART_TABLE."vocalmusic_student_select` 
	 		LEFT JOIN ".$TABLE."student_sheet ON ".$ART_TABLE."vocalmusic_student_select.student_number = ".$TABLE."student_sheet.number 
	 		LEFT JOIN ".$TABLE."major ON ".$TABLE."student_sheet.profession = bysj_major.name 
	 		WHERE `".$instrument."_finally`= '".$teacher_id."' AND ".$ART_TABLE."vocalmusic_student_select.year = '".$year."' AND ".$TABLE."major.id='".$major."' ORDER BY  `".$instrument."_lock` DESC  ";
