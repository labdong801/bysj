...
<?php
   $welcomestr = '&nbsp;&nbsp;&nbsp;欢迎'.$com_from.'<b>'.$com_name. '</b>同学使用艺术系双选系统'."<p>";
	...
	$sql = "select topic_start,topic_end,student_start,student_end,teacher_start,teacher_end from ".$ART_TABLE."set_date where grade = '3'";
	   ...
	   if($grade < 3)
	   {
	   		Show_Message("专业方向选修只对大三学生开放。<br>
		           感谢期待这门课程的同学们。");
			@include($baseDIR."/bysj/inc_foot.php");
           exit;   //不能选题，退出
	   }
...
$sql= "SELECT * FROM `".$ART_TABLE."major_student_select` WHERE `student_number`='$com_online' ";
...
for($tmpi=1;$tmpi<4;$tmpi++){
	...
	//else $dd = $wishn;
	  ...
	  if($result[$tmpi] > 0)
      {
      	echo "<img id='".$tmpi."' class='select' style='width:100px;height:100px;' src='..".getmajor($result[$tmpi],"icon")."'/>";
      }
      else
      {
      ...
      echo "<span class='select".$tmpi."'>".getmajor($result[$tmpi],"name")."</span>";
      ...
      if($result[$tmpi] > 0)
      {
      	echo "<span class='select".$tmpi."'>".getmajor($result[$tmpi],"detail")."</span>";
      }
      else
      {
      	echo "未选择";
...
if($currrows<=0)   {
 ...
 echo "<form>";
   ...
   for($i=0;$i<$currrows; $i ++){
	...
	$year = $year['year'];
	//echo $sql;
	$sql = "SELECT * FROM `".$ART_TABLE."major_student_select` WHERE first = '".$array['id']."' AND year = '".$year."' ";
	$hot_query=mysql_query($sql);
	$hot_first = mysql_num_rows($hot_query);
	$sql = "SELECT * FROM `".$ART_TABLE."major_student_select` WHERE second = '".$array['id']."' AND year = '".$year."' ";
	$hot_query=mysql_query($sql);
	$hot_second = mysql_num_rows($hot_query);
	$sql = "SELECT * FROM `".$ART_TABLE."major_student_select` WHERE third = '".$array['id']."' AND year = '".$year."' ";
	$hot_query=mysql_query($sql);
...
?>
	<br>&nbsp;<br>&nbsp;&nbsp;&nbsp;&nbsp;
   恭喜您，<b><?php echo $row["name"];?></b> 老师已经同意指导您的专业方向选修。以下是老师的联系方式。<br>&nbsp;<br>
...
function getmajor($id,$type)
{
	if($id > 0)
	{
		global  $ART_TABLE;
		$sql="SELECT * FROM  `".$ART_TABLE."major` WHERE `grade`=3 && `id`=".$id;
		$query = mysql_query($sql);
...
?>
	$(".select").click(function(){
		if($(this).attr("id") == 1)
		{
			s1 = 0;
			$(".select1").html("");
			$.post("./ajax/update_grade3.php", { select: "first", value: "0" ,number:"<?php echo $com_online;?>"});
		}
		if($(this).attr("id") == 2)
		{
			s2 = 0;
			$(".select2").html("");
			$.post("./ajax/update_grade3.php", { select: "second", value: "0" ,number:"<?php echo $com_online;?>"});
		}
		if($(this).attr("id") == 3)
		{
			s3 = 0;
			$(".select3").html("");
			$.post("./ajax/update_grade3.php", { select: "third", value: "0" ,number:"<?php echo $com_online;?>" });
		}
	...
	$(".point").click(function(){
		...
		if(s1==0)
		{
			...
			else
			{
				...
				temp = $(this).attr("src");
				$("#1").attr("src",temp);
				$.post("./ajax/update_grade3.php", { select: "first", value: s1 ,number:"<?php echo $com_online;?>"});
			}
		}
		else if(s2 == 0)
		{
			...
			else
			{
				...
				temp = $(this).attr("src");
				$("#2").attr("src",temp);
				$.post("./ajax/update_grade3.php", { select: "second", value: s2 ,number:"<?php echo $com_online;?>"});
			}
		}
		else if(s3 == 0)
		{
			...
			else
			{
				...
				temp = $(this).attr("src");
				$("#3").attr("src",temp);
				$.post("./ajax/update_grade3.php", { select: "third", value: s3 ,number:"<?php echo $com_online;?>"});
			}
		}
		else
		{
